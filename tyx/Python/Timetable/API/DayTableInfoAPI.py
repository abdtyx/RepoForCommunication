...
def GetDayTable():
    ...
    with open(path + 'time.txt', 'r') as timer1:
        ...
        if (tm1[0] == str(localtime.tm_year) and tm1[1] == str(localtime.tm_yday)):
            ...
            Table(cookies)
        with open(path + 'Table.txt', 'r') as tb2:
    ...
    with open(path + 'time.txt', 'r') as timer2:
        ...
        if (tm2[-1] != str(localtime.tm_wday)):
            ...
            with open(path + 'Table.txt', 'r') as ChangedTable:
                weektable = ChangedTable.readlines()
    # 注意此处从文件读入并返回无法直接给list，建议使用split分割得到的数据，也就是说依然需要一个Handle函数来split一下这个数据然后再返回给main一个列表
    try:
        return Split(weektable[localtime.tm_wday])
    except:
        return ['-1',]